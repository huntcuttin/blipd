...
2. #20 fixed (Resend errors surfaced, not logged as "sent") + bounce visibility — **#20 fixed 2026-08-02; bounce visibility code + DB migration done 2026-08-02 evening (see Infrastructure Limits below for the webhook spec), only the Resend dashboard webhook registration is left — no API path for that step**
...
- **Other UX gaps flagged by the founder during this walkthrough, not yet fixed**: no back button anywhere in the onboarding flow; console preference is single-select with no way to indicate owning both a Switch and a Switch 2; the "Games you own" picker (`getPopularGames()`) doesn't appear to apply the Game Quality tier ranking (see "Game Quality & Catalog Ranking" above) or prioritize Nintendo first-party titles — worth checking whether `getPopularGames` should route through `getGameTier()`/`computeTrendingScore` the same way `/sales` now does.
