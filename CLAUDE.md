...
## Session Log — 2026-08-05 morning (founder live feedback round + Nintendo slate reconciliation)

Founder reviewed the overnight work on device and sent fixes; all shipped same morning (commits `947c6d4`, `6669ddf`, `953ed8b`):

- **Release-time page CTA is now follow-state-aware** (`src/components/ReleaseTimeCta.tsx`): a user already watching sees a live "Watching" status card, never a "Watch & Get Notified" pitch. The page is SSR for SEO, so the CTA is a small client component reading FollowContext.
- **"Price TBA" instead of "Free"** on unreleased games with $0/$0 price data (Fortune's Weave read as a free game). "Free" now only renders for genuinely released $0 titles, matching GameCard's existing rule.
- **"What time does it launch?" and "I own this game" are pill buttons now**, not text links. Founder: nothing should look like a website link.
- **Alerts empty state `&apos;` glitch fixed** (HTML entity inside a JS string renders literally; use real apostrophes in JSX expression strings).
- **No-em-dash rule**: swept ~60 user-facing strings across 24 files (titles to " · ", subjects to ": ", prose to periods/commas). Rule added to the Bible's non-negotiables. Comments, tests, regex dash-classes, and the bare "—" price placeholder are exempt.
- **Nintendo slate reconciled against the eShop** (`fixes/sync_nintendo_first_party_slate.py`): founder supplied Nintendo's own New Releases + upcoming listings as ground truth (22 titles). 19 rows corrected/confirmed with Nintendo's dates, 8 created (Star Fox 6/25, Fortune's Weave base 9/17, Switch Sports Resort 10/22, XC3 S2E 12/3, Ocarina of Time 2026, Xenoblade Genesis / Pokémon Winds / Waves 2027, FireRed/LeafGreen 2/27). Notable finds: Rhythm Heaven Groove and Yoshi had shipped months ago but sat on TBA placeholders; **the "(English) ..." language-prefix filter in ingest permanently excludes legitimate titles like Pokémon FireRed/LeafGreen from ever entering via catalog sync** (inserted manually with stripped display titles; sync won't duplicate them since it keeps filtering the prefixed originals). Two matcher lessons baked into the script's comments: accent-fold before comparing (Pokemon vs Pokémon), and strip ™ BEFORE NFKD normalization (NFKD decomposes ™ into the letters "TM", silently breaking every ™-carrying match).
- **Upcoming page now leads with a "● Nintendo" rail** (red dot, renamed from "Nintendo · Up Next" per founder) pulling upcoming Nintendo IP from both the dated and TBA pools, flagship-tier first — Zelda/Pokémon stay top-of-mind even without firm dates. Out Now beneath it now mirrors Nintendo's own storefront list (Splatoon Raiders, Xenoblade 2 S2E, Fitness Boxing 3 leading). Verified live post-deploy.
- Tomodachi Life added to the A-tier IP list in `ranking.ts`.

**Still open for the founder** (unchanged from the overnight log): landing headline is deals-first vs the Bible's launch-first positioning; whether franchise-triggered alerts should appear in the in-app feed; signed-in Home visual check on a real device.

## Session Log — 2026-08-05 (audit round 2 + mobile app prep)

Founder asked for a full codebase re-audit and to start preparing the mobile app. Ran as 6 parallel background agents (4 audit dimensions, 2 mobile), every critical + top moderate then re-verified against code by the orchestrating session. **Full findings: `docs/AUDIT-2026-08-05.md`** (synthesis + execution order) with per-dimension detail in `docs/audit-2026-08-05/`. **Mobile: `docs/MOBILE-APP-PLAN.md`** — verdict: keep the `mobile/` Expo scaffold as a frame, re-port its content layer wholesale; email-OTP auth recommended; Expo Push Service over APNs; two submission blockers (no account deletion anywhere in the product, legacy anon key + stale dep matrix in the scaffold).

- **4 verified criticals**, all new since the 08-03 audit: Resend quota errors don't trip the dispatch breaker (C1) + failed sends still marked dispatched (C2) — together a quota-exhausted day permanently loses every pending alert; the pricedUpcoming alert-history guard is unpaginated and silently fails at exactly the recovery-wave scale it was built for (C3); alert dismissal is lost if the user navigates during the 5s undo window (C4). The 08-03 fixes themselves held (junk filters, zero-price guard, breaker, auth guards, suppression — all verified intact).
- **One fix shipped this session** (`cbe8d48`): the slate script's `release_date_source='nintendo'` rows weren't in catalog sync's trusted-restore list — the founder's 08-05 morning date fixes would have been reverted by the very next nightly sync (deployed ~20 min before it ran). Also finished audit #32's incomplete fix: `git rm -r --cached supabase/.temp` (was gitignored but still tracked/published). Everything else is report-only, awaiting the founder's go.
- **Highest-leverage single item found** (in the mobile gap analysis, but it's a web bug today): `insertAndDispatch` doesn't dispatch — delivery waits for the 10-min cron, so a launch alert can lag ~12 min behind burst-poll's ~2-min detection. Undermines the launch-minute differentiator on every channel; fix is one inline `dispatchRecentAlerts()` call after burst-poll fires.
- **Tooling/process**: the Workflow orchestration tool is unusable in this repo — two full runs had all 9 agents killed by its 180s no-progress watchdog regardless of model (~17M tokens for zero results). Plain background agents writing reports to files worked first try. Related: CLAUDE.md's size is now an operational cost (it's part of why agent turns run long) — a consolidation pass moving resolved session logs into docs/ would help every future session.

## Session Log — 2026-08-05/06 (audit round 2 fix execution)

Founder said "fix the bugs now" after reviewing `docs/AUDIT-2026-08-05.md`. All 4 criticals, all verified moderates, and the cheap minors shipped in 10 verified batches (tsc + tests + build before every commit, one concern per commit): `a571874` dispatch reliability (quota errors trip the breaker; tri-state send outcomes so failed sends retry next tick while suppressed/no-channel recipients still count done; `sendAlertToUsers` checks the breaker between batches; 72h staleness valve per late-is-worse-than-never), `ac10883` paginated false-out-now guard (fails safe on query error), `2256610` dismissals commit on unmount, `c6e14d5` four more unbounded-select fixes (+ dispatch aborts on sent-pairs lookup failure rather than risking duplicates), `973f22e` alert-correctness moderates (flap dedup covers price_drop; sale_ending fires once per sale via sale_end_date dedup + Pacific window; Launch Burst Poll monitored; stuck-placeholder uses created_at; sync-release-dates no longer stamps out_today; pastRelease skips sentinel dates), `5830631` UX/data moderates (getUserProfile error bias, franchise-page junk filter, feed/alerts loading flashes, negative countdowns, onboarding error surfacing, ilike escape), `cedc456` launch alerts dispatch inline from burst-poll (kills the ~12-min lag on the differentiator) + explicit logged push-endpoint handoff, `9231b39` 'manual' added to trusted date sources, `f4f4125` minor sweep (9 Watch-verb copy stragglers, 4 touch targets, targetPrice leak on unfollow, IGDB 429 gap in getIGDBHype's direct-ID path), `b9a32e7` stuck-placeholder scoped to upcoming/out_today (would have paged about the known 66-row TBA residual otherwise — verified live before shipping).
