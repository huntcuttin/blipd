...
- **Checked how far back this goes**: grouped every false `out_now` by calendar day with no lower bound — **only 2026-08-03 shows any hits, zero on any prior day.** This is reassuring in scope: not a chronic, months-old leak, but concentrated specifically on today, consistent with today being a day Nintendo's backend had an unusually broad, temporary pricing data-quality lapse (matching the same "platform occasionally serves bad data widely, not just to one title" pattern already documented for both the price API and YouTube's RSS endpoint tonight).
- **Final tally, fully reconciled**: 40 false `out_now` alert rows total today, 8 real incorrect emails sent to the founder (all Monster Hunter or Dragon Quest titles — his own followed franchises), 32 rows that were correctly never dispatched (all now deleted). The fix (`5359221`, confirmed deployed and working via `3c5404a`) stops this going forward regardless of *why* a game's price was ever $0 — it doesn't matter whether the founder's own session caused the recovery or the pipeline did it on its own.
