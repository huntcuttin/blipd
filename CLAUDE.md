...
- **Verification caveat**: this fix is applied with high confidence based on deterministic, well-documented Postgres RLS behavior (not guessed), but a full end-to-end confirmation (an actual authenticated browser request succeeding) still needs a real user session — couldn't safely fabricate a JWT to test this without touching auth internals overnight. Worth a 10-second manual check next time you're in Settings or a game detail page: toggle a per-game notification preference off, refresh, confirm it stayed off.
