...
**#11 — durable dispatch, partial, blocker documented per the founder's own instruction** (`679cb79`). Three real gaps: the alerts select had no `.range()` (PostgREST's 1,000-row cap could silently truncate a busy window), the 15-min lookback drops an alert forever if the dispatch cron is ever down/erroring past that, and a Resend rate-limit error was logged-and-ignored rather than stopping the run. **The audit's ideal fix (a persisted watermark or a `dispatched_at` column for true undelivered-work tracking) could not be shipped this pass — every path to executing schema DDL is currently blocked**: the Supabase Management API token is expired (confirmed via a direct API call), there's no `DATABASE_URL`/direct Postgres connection string anywhere in `.env.local`, the registered Supabase MCP server fails to connect (needs `SUPABASE_ACCESS_TOKEN`, unset), and the Supabase CLI is unauthenticated (`supabase projects list` → `Unauthorized`) — all four dead ends trace to the same root cause. **Needs a fresh personal access token from supabase.com/dashboard/account/tokens before the real fix can ship.** Shipped the schema-free version instead: paginated the alerts fetch, widened the window 15min→3h (safe because the existing `alreadySentPairs` check already makes re-fetching idempotent — nothing double-sends), and added `rate-limit.ts`, a shared flag every Resend send site sets on `rate_limit_exceeded` so `dispatch.ts` stops issuing further sends for the rest of that run; the un-sent alerts simply retry whole on the next 10-min tick once the limit has reset. **Could not confirm this deployed live** — the Vercel API also has no auth token in this environment (`api.vercel.com` returns `missingToken`), and `vercel whoami`/`vercel deploy` both fail unauthenticated too, so "pushed to `main`" is as far as this session can verify; the SHA-matches-HEAD check the founder's own discipline rules require just isn't possible right now without dashboard access.
...
Build + `tsc --noEmit` + `next build` clean before every commit above. Remaining Phase 1 items (#12 event-creation breaker/ceiling, #16 sync-release-dates no-match marker, #17 suppression semantics split, #18 sentinel/predicate consistency, #19 smaller alert-path items) still pending — continuing in this same loop.
