...
**Important correction to the false out_now incident above: the earlier "10 total" count was itself incomplete, found via an inbox spot-check, not an exhaustive query.** Found via a fresh, unrelated inbox check: two more real, already-sent false emails — "DRAGON QUEST - HD-2D Erdrick Trilogy Collection" (07:50 UTC) and "Monster Hunter Rise + Sunbreak" (07:50 UTC) — that the earlier investigation's reactive, inbox-driven approach missed entirely. Ran a proper exhaustive query this time across the full incident window (07:20–08:05 UTC, covering both the pre-fix 07:50 tick and the deploy-race 08:00 tick) instead of trusting a partial inbox search: **27 total false `out_now` rows, of which 5 were actually dispatched and emailed** (not 3): Monster Hunter Stories, Monster Hunter Stories 2: Wings of Ruin Deluxe Edition, Monster Hunter Stories 2: Wings of Ruin Deluxe Kit, Monster Hunter Rise + Sunbreak, and DRAGON QUEST - HD-2D Erdrick Trilogy Collection. All 5 match franchises the founder actually follows (Monster Hunter, Dragon Quest) — which is exactly why these 5 specifically got emailed while the other 22 (Capcom Arcade Stadium, DRAGON BALL, ACE COMBAT, Street Fighter 6, etc. — franchises he doesn't follow) correctly never dispatched.
- **Cleaned up the 22 never-dispatched rows** directly (confirmed each had no `notification_log` entry first). Left the 5 already-emailed rows alone as an honest historical record — those emails can't be un-sent regardless, and attempting to delete an alert row with a real `notification_log` reference risks an FK conflict for no benefit.
- **Lesson**: when investigating "did X false thing happen," query the database exhaustively for the *actual* condition (here: `type='out_now' AND prior alert history exists`) across the *full* suspect time window, rather than reacting only to whatever happened to surface in a spot-check (an inbox search, a sampled query). The spot-check found real problems both times, but undercounted by more than half — 3 of 5, in this case. This complements (doesn't replace) the fix itself, which is confirmed already deployed and working going forward from `62fef89`/`3c5404a` onward.
