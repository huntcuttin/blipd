...
Build + `tsc --noEmit` + `next build` clean before every commit above, each spot-verified against the live REST API where the change touched a query's filter logic. Remaining Phase 1 items (#16 sync-release-dates no-match marker, #19 smaller alert-path items) still pending — continuing in this same loop.
